...
function orderTotal($orders)
{
    ...
    return $orders->whereNotIn('id', $remIds);
}


//////////////////////////////////////////////////////////////////
function updatePaymentStatus($info): \Illuminate\Http\JsonResponse
{
    if ($info) {
        $status = $info->payment_status;
        if ($status == 0) {
            $info->update(['payment_status' => 1]);
        } else {
            $info->update(['payment_status' => 0]);
        }
        return response()->json(['status' => 'success', 'message' => trans('تم تعديل حالة الدفع بنجاح.'), 'type' => 'no']);
    } else {
        return response()->json(['status' => 'error', 'message' => trans('لم يتم العثور على البيانات!')]);
    }
}
