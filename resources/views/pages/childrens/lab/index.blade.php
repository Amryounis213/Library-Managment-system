...
{{--                        <!--begin::Stat-->--}}
{{--                        <div--}}
{{--                            class="{{$order->lab_payment <= 0 ? 'alert-danger' : 'alert-success'}}  border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3 text-center">--}}
{{--                            <h3 style="color: {{$order->lab_payment <= 0 ? 'red' : 'green'}}; margin: 10px 0px;">{{$order->lab_payment <= 0 ? 'غير مدفوع' : 'مدفوع'}}</h3>--}}
{{--                            <!--end::Label-->--}}
{{--                        </div>--}}
                        <!--end::Stat-->
                    </div>
        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div class="card-header border-0 cursor-pointer" role="button"
                ...
                <div class="card-title">
                    ...
                    <h3 class="fw-bolder m-3">{{ __('Lab') }} </h3>
                    <!--end::Search-->
                </div>
            </div>
            <!--begin::Card header-->
            <!--begin::Content-->
            <div id="kt_laboratory_details" class="show collapse">
                <!--begin::Card body-->
...
{{--                            <th>حالة الدفع</th>--}}
        ...
        <div class="card-footer d-flex justify-content-end py-6 px-9">
            <a type="button" class="btn btn-secondary m-1 w-125px" href="{{ route('order.index') }}">رجــــوع</a>
            <a type="button" class="btn btn-primary m-1 w-150px"
               href="{{ route('order.checkup.print', ['type' => 'all' ,'order' => $order]) }}">طباعة
                الكل</a>
            <a type="button" class="btn btn-warning m-1 w-150px"
               href="{{ route('order.checkup.print', ['type' => 'out' ,'order' => $order]) }}">طباعة
                النواقص</a>
            @if($order->status > 3 && $order->lab_payment > 0)
                <a type="button" class="btn btn-success m-1 w-150px"
                   href="{{ route('order.checkup.print', ['type' => 'in' ,'order' => $order]) }}">طباعة
                    المستلمة </a>
            @endif
            @if( $order->lab_payment < 0)
            <!--begin::Form-->
                <form id="details_form" class="form" method="POST" action="{{ route('order.status.update') }}"
                    ...
                    <input type="hidden" name="lab_payment" value="0">
            ...
            {{--                    @method('POST')--}}
            {{--                    <input type="hidden" name="order_id" value="{{$order->id}}">--}}
            {{--                    <input type="hidden" name="in_lab" value="2">--}}
            {{--                    <button type="submit" class="btn btn-danger m-1 w-125px" id="kt_account_profile_details_submit">--}}
    ...
    @section('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                //////////////////////////////////////////////
                let checkupsTable = $('#data_table').DataTable({
                    "processing": true,
                    "serverSide": true,
                    "info": false,
                    "paging": false,
                ...
                $(document).on('click', '.sts-fld', function (e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('order.checkup.status') }}",
                        data: {'id': id},
                        success: function (data) {
                            totalPrice();
                            if (data.type === 'yes') {
